...
st.markdown("This component runs FFmpeg commands entirely in your browser. No data is uploaded.")

# Display file size recommendations
st.info("""
**File Size Recommendations:**
- **Desktop:** Up to 100MB for smooth performance.
- **Mobile:** Up to 20MB is recommended, as mobile browsers have less memory.
""")
...
if f:
    ...
    if file_size_mb > 100:
        ...
        st.warning(f"File size: {file_size_mb:.1f}MB. Performance may be slow on mobile devices.")
    else:
    ...
    operation = st.selectbox("Choose an operation", [
        "Trim Video", "Convert to Grayscale", "Convert to GIF", "Extract Audio (AAC)"
    ])
            ...
            with st.spinner('Processing in browser... Please wait.'):
                try:
                    ...
                    if out_data:
                        ...
                        elif output_type == "image": st.image(out_data)
                        elif output_type == "audio": st.audio(out_data, format=output_mime)
                        st.download_button('Download Output', out_data, output_filename, mime=output_mime)
                    else:
                        st.warning("Component did not return data. The operation may have been cancelled.")
